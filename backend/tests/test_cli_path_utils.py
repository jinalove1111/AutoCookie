"""Tests for scripts/_cli_path_utils.py (Milestone 39, `ENGINEERING_DECISIONS.md` #77).
...
`scripts/` for the same `db_path = Path(args.db_path)` pattern found
five call sites sharing this bug -- fixed once in
`scripts/_cli_path_utils.normalize_db_path_arg` and imported by all
five (`cto_report.py`, `selector_dry_run.py`, `shadow_status.py`,
`migrate_paper_db.py`, `paper_trader_health_check.py`) instead of five
separate copies. These tests check the string transformation directly
so the behavior is verified without depending on which OS actually
runs pytest.
"""
...
from _cli_path_utils import normalize_db_path_arg  # noqa: E402


def test_converts_windows_backslash_path_with_drive_letter_to_posix_form():
    result = normalize_db_path_arg("C:\\Users\\test\\paper_validation.db")
    assert result == Path("C:/Users/test/paper_validation.db")


def test_converts_relative_backslash_path():
    result = normalize_db_path_arg("backend\\paper_validation.db")
...
def test_leaves_plain_filename_untouched():
    result = normalize_db_path_arg("paper_validation.db")
...
def test_mixed_separators_left_untouched_not_misinterpreted():
    ...
    result = normalize_db_path_arg("some/dir\\odd_name.db")
