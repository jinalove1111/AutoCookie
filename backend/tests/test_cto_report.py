...
# --------------------------------------------------------------------
# Regression: real 2026-07-17 production failure. Sections 5/6 (Strategy
# Rankings, Shadow Performance) degraded to "unavailable: could not
# collect regime evidence ... (Could not parse SQLAlchemy URL from given
# URL string)" whenever the invoking shell has no `DATABASE_URL`
# configured (no `backend/.env`, no exported env var) -- root cause:
# `collect_regime_evidence` -> `_collect_shadow` LAZILY imports
# `app.portfolio.shadow_resolver` (for its `RESOLUTION_MODEL` constant)
# -> `app.portfolio.trades` -> `app.database.session`, whose
# MODULE-LEVEL `create_engine(settings.DATABASE_URL, ...)` raises when
# that setting is `""` (`app/config.py`'s documented default) -- entirely
# unrelated to this report's own read-only `mode=ro` session
# (`_connect_readonly_session`), which never imports
# `app.config.settings`/`app.database.session` at all. This never
# surfaced in the rest of this suite because `fresh_app_env` (via
# `db_session`/`migrated_db`/`app_main`) always sets `DATABASE_URL`
# BEFORE the first import of `app.database.session` in the pytest
# process -- these tests deliberately purge that module (and its lazy
# import chain) and force `settings.DATABASE_URL` empty first, so the
# exact cold-import, no-env failure mode from the real invocation is
# reproduced here rather than masked by fixture ordering.
# --------------------------------------------------------------------
...
def test_ensure_unrelated_write_engine_importable_sets_placeholder_when_unset(monkeypatch):
    ...
    cto_report_script._ensure_unrelated_write_engine_importable()
...
def test_cli_renders_rankings_and_shadow_when_database_url_unset_regression(
    argv_style, db_session, sqlite_url, tmp_path, monkeypatch, capsys
):
    from app.database.models import RegimeSnapshot, ShadowSignal
    ...
    db_session.add(
        ShadowSignal(
            ...
            # only counts rows resolved under the CURRENT model (see
            # app.portfolio.rolling_regime_performance's own module
            # docstring); a NULL/legacy value here would silently be
            # excluded and this test would fail to observe any evidence
            # regardless of the DATABASE_URL fix under test.
        )
    )
    db_session.commit()
    ...
    output_path = tmp_path / f"cto_report_no_database_url_{argv_style}.md"
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["cto_report.py", db_path_arg, "--output", str(output_path), "--since", "24 hours ago"],
    )
    ...
    # Before the fix: both sections degraded to "unavailable: could not
    # collect regime evidence ... (Could not parse SQLAlchemy URL from
    # given URL string)". After the fix: real evidence renders.
    assert "Could not parse SQLAlchemy URL" not in captured.out
    assert "strategy | shadow_n | live_n | sufficient_cells" in captured.out
    assert "Total resolved shadow samples (tp/sl outcomes): 1" in captured.out
