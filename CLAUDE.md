...
| Plan for real exchange order-placement infrastructure (Gate #4) | `docs/EXCHANGE_LAYER_IMPLEMENTATION_ROADMAP.md` (Phase 0 read-only methods implemented+tested with mocks, milestone 37; Phase 1+ order placement still needs approval + real credentials) |
...
a CTO platform evaluation (milestone 35,
`docs/CTO_PLATFORM_EVALUATION.md`), a five-priority CTO round (milestone
36 — Exchange Layer roadmap, research-platform ranking, OSS comparison,
`docs/EXCHANGE_LAYER_IMPLEMENTATION_ROADMAP.md`), and an operator-approved
...
see `ENGINEERING_DECISIONS.md` #76), the CI mystery's actual resolution
(milestone 39 — root-caused to a genuine Windows-vs-Linux `pathlib` bug
affecting five `scripts/` entry points, fixed once in
`scripts/_cli_path_utils.py`; see `ENGINEERING_DECISIONS.md` #77), and
that same fix generalized plus a real monitoring gap closed (milestone
