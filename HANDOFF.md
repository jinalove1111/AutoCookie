...
## 상태: (야간 CTO 세션, founder 부재, 계속 진행 중) 3번째 자산(SOLUSDT) 6개월 딥 재검증까지 완료. **누적 결과**: break-even은 BTC +9.2% / ETH -1.9% / SOL -4.8%로 **3개 자산 중 2개에서 부정적** — "자산-의존적이라 어느 쪽이든 갈 수 있다"는 기존 결론에서 한 단계 더 나아가 "지금까지 검증한 자산에서는 긍정보다 부정이 더 흔하다"로 결론이 강화됨(단, 3개는 여전히 적은 자산 표본). SOLUSDT의 break-even 결과는 ETHUSDT처럼 혼재된 게 아니라 균일하게 flat-to-negative(6개 중 0개 개선/4개 악화/2개 무영향). 반면 **Breaker Block**(BTC -3.8%/ETH -12.0%/SOL -1.9%, 3개 자산 전부 부정)과 **Partial TP**(BTC -32.6%/ETH -35.4%/SOL -29.1%, 3개 자산 18/18 기간 전부 악화, 프로젝트에서 가장 견고한 발견)는 3개 자산 모두에서 일관되게 재현됨. 코드 변경 없음(순수 재검증 라운드). `ENABLE_BREAKEVEN` 기본값 False는 그대로 유지 — "기본값을 True로 바꿀지 재검토"는 4번째 자산 결과가 나올 때까지 보류. Live 관련 코드는 여전히 전무 — Small Live는 operator의 명시적 승인 대기 중
...
## 현재 위치
Strategy > Risk > Backtest > Paper Trading > Dashboard 전 계층의 배관 갭은 전부 해소됨. 감사 HIGH 항목 3개 전부 A/B 검증 완료, 이제 BTCUSDT/ETHUSDT/SOLUSDT 세 자산 모두 6개월 딥 데이터로 재검증까지 완료: **break-even**(BTC +9.2%/ETH -1.9%/SOL -4.8% — **3개 중 2개 부정**, "자산-의존적"에서 "긍정보다 부정이 더 흔함"으로 결론 강화)/**Partial TP**(BTC -32.6%/ETH -35.4%/SOL -29.1% — 3개 자산 18/18 기간 전부 악화, 프로젝트 최강 근거)/**Breaker Block**(BTC -3.8%/ETH -12.0%/SOL -1.9% — 3개 자산 전부 부정). `ENABLE_BREAKEVEN`은 paper trading에 배선은 됐지만 기본 False 유지 — 이번 결과가 그 선택을 더 강하게 뒷받침함(기본값을 True로 바꾸는 건 4번째 자산 결과 나올 때까지 보류). **다음 최고-ROI 후보 (우선순위순)**:
- **4번째 자산 확보(가급적 진짜 상관성 낮은 자산 클래스)**: BTC/ETH/SOL은 전부 대형 L1이라 여전히 서로 비슷한 베타 — break-even의 "2/3 부정"이 4번째 자산에서 또 뒤집힐 수 있음(1/2였던 BTC-only 결과가 ETH 추가로 뒤집혔던 것과 같은 패턴)
- **다른 연도로 확장**: 지금까지 세 자산 모두 2026년 1~7월이라는 동일 캘린더 구간만 검증함(자산-일반화는 테스트했지만 시간-일반화는 아직)
