...
## 71. Milestone 33: Validation Phase begins -- a critical exit-check bug found and root-caused, Gate #4's latency infrastructure confirmed not to exist yet, and a real timeframe-configuration ambiguity surfaced

**Decision context**: `docs/PHASE_TRANSITION_REVIEW.md` recommended
pausing the default hypothesis-research cadence and verifying the
paper-trading pipeline instead. Operator directive: "You are entering
Validation Phase... verify the paper trading pipeline end-to-end...
measure end-to-end execution latency... identify any reliability,
synchronization, logging, or execution issues." This decision records
what that verification found, why it matters, and why nothing was fixed
in the same round.

**New tooling** (all additive, read-only relative to production code):
`scripts/measure_pipeline_latency.py` (calls `run_paper.run_once()` and
`CandleFetcher.fetch_ohlcv` directly, unmodified, within one warm
process -- matching real loop-mode per-iteration cost, not N separate
process launches); `scripts/verify_signal_to_fill.py` (injects one
deterministic synthetic `TradeSignal` via `unittest.mock.patch` in place
of `SignalEngine.generate_signal()`'s real market-dependent output, then
lets the REAL, unmodified signal -> risk -> execute -> persist -> close
chain run against a throwaway temp SQLite DB created by the REAL
`alembic upgrade head` path, never the production DB);
`backend/tests/test_run_paper_exit_check.py` (the first-ever automated
test for `scripts/run_paper.py`'s own orchestration logic, using the
same `migrated_db` fixture every `app.portfolio.*` test already uses).

**Finding #1 (CRITICAL) -- the exit-check step crashes and silently
halts the pipeline the first time a real trade's stop-loss or
take-profit is actually reached on a later pass than the one that opened
it.** Root cause: `Trade.opened_at` is declared `DateTime(timezone=True)`
(`backend/app/database/models.py`), but SQLite's SQLAlchemy dialect does
not preserve timezone-awareness across a write/read round-trip --
`TradeTracker.get_open_positions()` returns `opened_at` as
timezone-NAIVE regardless of the column's declared type.
`_check_and_close_open_positions()` (`scripts/run_paper.py` ~line 288)
computes `closed_at = datetime.now(timezone.utc)` (timezone-AWARE) and
at line 297 computes `(closed_at - opened_at).total_seconds()` for
`holding_time_seconds` -- raising `TypeError: can't subtract
offset-naive and offset-aware datetimes`. The crash happens BEFORE
`TradeTracker.close_trade()` is called (line 299), so the position is
NEVER actually closed, and `run_once()`'s own documented
one-trade-open-at-a-time concurrency guard then skips ALL signal
generation on every subsequent pass while any position remains open.
**The first time this triggers in real production, the paper trader
stops generating new signals, permanently, until manually intervened.**
Only triggers when an exit condition is actually met (a position that
never reaches its stop/target never hits this code path) -- consistent
with, and explaining, why the empty `trades` table (Finding #4) never
surfaced it before: no trade has ever been recorded as opened at all.

**Reproduced twice, independently, confirming one root cause**: (1) via
the real `run_once()` pipeline itself -- opened a synthetic trade
through the unmodified signal -> risk -> execute -> persist chain
against a throwaway temp DB, then called `run_once()` again; the
exit-check step that runs at the start of every pass crashed with the
exact `TypeError`; (2) via a direct call to
`_check_and_close_open_positions()` with a price forced past the
take-profit level -- identical crash, same line.

**Why this was never caught**: `scripts/run_paper.py` had NO direct
pytest coverage of any kind before this round (confirmed by exhaustive
search, matching `CLAUDE.md`'s own prior disclosure). Every individual
component this bug touches (`TradeTracker`, `PaperBroker`, `RiskManager`)
is well-tested in isolation -- 789 passing tests -- but the ORCHESTRATION
wiring between them, exercised only by actually running the script in
production, had never been exercised by an automated test until this
round added the first one.

**A permanent regression test was added**,
`backend/tests/test_run_paper_exit_check.py`, marked
`@pytest.mark.xfail(strict=True)` with the full root-cause explanation
inline. `xfail` (not a plain failing test) keeps the "full suite
passing" convention intact while making the bug permanently visible in
CI; it will flip to a hard failure if anyone accidentally "fixes" it
incorrectly (strict mode) and to a genuine pass once a real fix lands.

**Not fixed in this round.** `scripts/run_paper.py` is one of the two
files `CLAUDE.md` section 2 gates behind explicit operator sign-off
before modification, and this round's own instruction was explicit: "do
not modify production trading logic." Recommended fix direction, not
implemented: read `opened_at` back as UTC-aware explicitly (e.g.
`opened_at.replace(tzinfo=timezone.utc)` if naive) before the
subtraction -- the standard fix for this exact SQLAlchemy+SQLite class
of bug.

**Finding #2 (CRITICAL, unresolved) -- cannot confirm which timeframe
the real production process has actually used.** `Settings.DEFAULT_TIMEFRAME`
defaults to `"5m"` (confirmed both in source and at runtime during this
round's live measurements), and `.env.example` documents `5m` as the
standard value, not an accidental default. Nearly this entire project's
delay-fragility research -- the evidentiary basis for
`docs/live_trading_checklist.md`'s Gate #4 hardening -- was conducted at
`15m` (`docs/LEGACY_DELAY_ROBUSTNESS.md`, `docs/ATR_FLOOR_EVALUATION.md`,
every hypothesis H1-H8). The platform's very first profitability
validation (`docs/PROFITABILITY_EXPERIMENT_REPORT.md`/
`docs/ROBUSTNESS_REPORT.md`) was itself conducted at 5m -- the 5m-vs-15m
split reflects two genuinely different points in this project's history
that were never explicitly reconciled. `docker-compose.yml` points the
backend service at a `.env` file; none exists in this repository
(`.env`/`.env.local` are both gitignored by design), and the process
that would have used it is not currently running (Finding #4) -- there
is no way to inspect its actual launch environment from the repository
alone. If the real deployment has been running at 5m, "one candle of
delay" there is 5 minutes, not the 15-minute figure Gate #4's entire
hardened requirement is built around, and the strategy's own
timeframe-relative stop/target/structural-level computations would
differ meaningfully. Recommended: verify (and if necessary, align) the
real `.env`'s `DEFAULT_TIMEFRAME` against the 15m safety-research
standard, or re-run the core delay-fragility check at 5m if that is
confirmed to be the real deployed value. Not resolved in this round --
requires operator access to the actual deployment environment.

**Finding #3 -- Gate #4's "measured signal-to-fill latency" cannot be
produced by the current architecture at all.** Verified by direct source
inspection: `app.execution.paper_broker.PaperBroker` contains no
`requests`/`httpx`/`aiohttp`/network call of any kind (the module's own
docstring already states "No real HTTP/exchange calls anywhere in this
module"). The only real network call anywhere in the paper-trading
pipeline is `CandleFetcher.fetch_ohlcv()`'s OKX market-data fetch. Gate
#4's own hardened requirement cannot be satisfied by running or
measuring the EXISTING pipeline more carefully -- there is no real
exchange order-placement round-trip anywhere in this codebase to measure
yet. This is an infrastructure gap, not a measurement gap; clearing Gate
#4 on real evidence requires building a real (at minimum demo-mode)
exchange order-placement integration first.

**Finding #4 -- the paper-trading process was not observably running
during this validation.** No matching process for `PROJECT_STATUS.md`'s
last-recorded PID (24616, milestone 28). `regime_snapshots` activity
(populated every real pass) runs 2026-07-16 02:14:44 through 2026-07-18
00:41:05, then stops -- roughly 29-30 hours stale as of this validation.
Cannot distinguish an actual production outage from this review running
in a different environment than wherever the real process was deployed
-- flagged for operator confirmation, not asserted as an incident.
Separately: `trades`/`signals` are both completely empty (0 rows)
despite ~2 days of recorded activity -- no real Legacy trade has ever
opened in this database's recorded history, consistent with (not
necessarily contradicting) Legacy's known low signal rate.

**Finding #5 -- two real, disclosed dead observability tables.**
`strategy_logs` and `risk_events` are both real, migration-created
tables with thoughtfully-designed columns that no code anywhere writes
to (confirmed via exhaustive grep across `backend/` and `scripts/` --
zero matches beyond schema/migration definitions). Not fixed (would
require editing the gated `scripts/run_paper.py`) -- recommended as a
low-risk observability improvement for a future, explicitly
operator-approved change.

**Finding #6 -- signal -> risk -> execute -> persist math verified
correct.** Real reproduction, hand-checked against the actual formulas:
fill price (50,010.0, 0.02% unfavorable slippage), position size
(0.025, 0.25% risk / $1,000 stop distance), fee (0.625125, 0.05% flat
taker), slippage (10.0) -- all matched exactly. The open-side pipeline
is confirmed mathematically correct; Finding #1's bug is specifically in
the close-side exit-check step. Concurrency-guard verification is
INCONCLUSIVE, blocked by Finding #1 (the exit-check crash pre-empted the
guard's own logic on the second test pass) -- should be re-verified once
Finding #1 is fixed.

**Latency measured** (Finding #3's scope limit applied honestly, neither
number is real exchange order latency): OKX candle-fetch round-trip (10
live samples) median 107.3ms, p95/max 195.8ms; full `run_once()`
...
**Status**: read-only verification round for existing behavior, plus
new additive test/observability tooling -- `RiskManager.evaluate()` and
`scripts/run_paper.py` were read and run, never modified. No orders
placed, no writes to the production `backend/paper_validation.db` (all
synthetic-signal testing used throwaway temp SQLite databases). Full
...
## 74. Milestone 36: five-priority CTO round -- Exchange Layer roadmap, research-platform ROI ranking, OSS architecture comparison, targeted infra touch-ups, CI failure found and investigated (not silently left broken)

**Decision context**: operator directive combining five priorities in
one round, each with its own scope and approval boundary. Rationale for
each recorded here; full content in the dedicated docs cited below
(cite, don't duplicate).

**Priority 1 -- Exchange Layer roadmap** (`docs/EXCHANGE_LAYER_IMPLEMENTATION_ROADMAP.md`,
planning only, explicitly not implemented per the operator's own "Do
NOT implement yet" instruction). The single most important finding,
made BEFORE writing any plan content: `app.execution.live_broker.LiveBroker`'s
current stub methods (`place_order`/`cancel_order`/`get_balance`/
`get_open_positions`) match `BaseExchange`'s contract, not what
`ExecutionEngine`/`OrderManager` actually call
(`fill_entry`/`check_exit`, matching `PaperBroker`) -- even though
`LiveBroker`'s own docstring already states its purpose as "matching the
paper broker's interface shape." This is not a redesign decision; it is
recognizing that `LiveBroker` needs to be an ADAPTER wrapping a
`BaseExchange` instance internally, translating between the two shapes
-- exactly satisfying "reuse the existing interfaces, do not redesign
them," since every other file in the chain (`ExecutionEngine`,
`OrderManager`, `safety_checks.verify_safe_to_trade`,
`RiskManager.evaluate()`) needs zero changes under this reading. Full
roadmap: 4-phase rollout (demo-API read-only -> demo-API order placement
-> default-off `run_paper.py` wiring -> real capital), each phase its
own approval gate, covering authentication (OKX's existing
HMAC-SHA256-signed-header scheme, reusing already-gitignored credential
storage verbatim), order lifecycle (idempotent `clOrdId`, a real design
decision on resting SL/TP orders vs. polling), position synchronization
(a genuinely new concern `PaperBroker` never had), websocket/data flow
(deferred until a first REST-based number exists to judge against),
retry logic (found `CandleFetcher`, the one real working network call in
this codebase, has NO retry/backoff at all -- a pre-existing gap, not
new), error recovery (fail-closed, reusing existing alert functions),
risk gates (explicitly unchanged -- `RiskManager.evaluate()` stays the
single gate), testing strategy (generalizing the validation phase's own
`verify_signal_to_fill.py` pattern), and rollback strategy (the new
Phase-2 flag IS the rollback mechanism, mirroring every existing
opt-in-default-off backtest flag's discipline).

**Priority 2 -- research-platform ROI ranking**
(`docs/RESEARCH_PLATFORM_ROI_RANKING.md`, deliberately narrower than
Milestone 35's whole-platform ranking, does not re-rank it). Top 10
ranked by Impact/Cost/Risk/Time/Business-Value. Two Immediate items
implemented this round, both pure documentation, zero production-code
touch: `docs/EXPERIMENT_INDEX.md` (every hypothesis H1-H8 indexed by
verdict/milestone/decision#/report -- built specifically so "has this
been tested" is a table lookup, not a re-grep of 300KB+ files, which
every prior round in this session had to do manually) and
`docs/HYPOTHESIS_BACKLOG.md` (every currently-available candidate
direction in one table). `scripts/shadow_status.py` was checked directly
before considering a shadow-data-monitoring item -- already fully covers
that need (Milestone 13), correctly not re-proposed, per "do not
recreate existing systems."

**Priority 3 -- OSS architecture comparison**
(`docs/OSS_AGENT_ARCHITECTURE_COMPARISON.md`), grounded in a live 2026
web search (CrewAI: 52,800+ stars/5.2M monthly downloads; LangGraph:
34.5M monthly downloads; AutoGen: merged into Microsoft Agent Framework,
now maintenance-mode; MetaGPT/ChatDev: full-organization simulation;
Dify: 144k stars) rather than solely prior training data.
Multi-agent-software-team frameworks (MetaGPT/ChatDev/CrewAI) duplicate
the capability this repo's own `.claude/` harness (CTO -> department
Heads -> sub-agents) already provides -- not adopted, per "do not
duplicate functionality already present." Autonomous-engineering-workflow
state management (LangGraph's durable state/checkpointing) is arguably
outperformed by this project's own append-only documentation discipline
for its SPECIFIC need (cross-session, human-readable, git-versioned
audit trail, not single-run execution state) -- not adopted. **One
genuinely missing capability, found and named precisely**: debate-based
decision verification. This project's own H7 and H8 results (decisions
#69, #70) already demonstrate the exact value this class of technique
provides -- a literal, mechanically-applied keep-rule result that needed
a second, skeptical pass (re-pooling `by_reason` by category instead of
exact string for H7; isolating which dimension actually drove H8's
PARAMETER_SENSITIVE label) before it could be trusted as the real
finding -- performed manually by one agent catching its own result,
not by two independent agents formally arguing. Recommended as a
lightweight PROCESS convention for a future hypothesis round (one
independent re-check of a literal keep-rule result before it is written
up as final), explicitly NOT a new dependency, framework, or `.claude/`
agent -- MCP (tool/data connectivity) noted as a real, current trend
worth watching for the eventual exchange-layer work, not worth adopting
for a single integration today.

**Priority 4 -- targeted infrastructure touch-ups, "never rewrite
working systems"**: `CLAUDE.md`'s milestone count (28 -> 35, a real,
meaningfully large staleness gap) and "where to look for X" table
(pointers to this round's new docs) updated -- narrow, targeted edits,
not a restructure. Agent memory updated (`jadecap-project-state.md`
brought current; new `gated-file-discipline.md` capturing this
session's clearest, most-repeated behavioral lesson: the
`scripts/run_paper.py`/`RiskManager.evaluate()` sign-off gate applies at
the FILE level regardless of how safe or small a specific change looks,
confirmed even for Milestone 34's 2-line bookkeeping fix). Agent
definitions reviewed via Priority 3's own comparison -- no changes
warranted, existing harness already sound. **CI, found broken this
round, investigated rather than left silently failing**:
`.github/workflows/backend-tests.yml` (added Milestone 35) fails at its
"Run test suite" step, exit code 1. Diagnosed as far as possible without
authenticated GitHub access: `gh` CLI is not installed in this
environment; unauthenticated REST API log download returns 403 ("Must
have admin rights"); unauthenticated page fetches surface only "Process
completed with exit code 1," not the underlying pytest output. The
single most likely hypothesis (dependency-version drift, since
`requirements.txt`'s own comment already discloses "exact patch
versions should be locked later... once the app is exercised end-to-end"
as a known, disclosed risk) was tested DIRECTLY, not assumed: a fresh
virtualenv, installed purely from `requirements.txt` with no other
history, resolves `pytest==8.4.2` (the locally-used dev `.venv` has
`9.1.1`, outside `requirements.txt`'s own declared `<9.0` cap) -- running
the full suite against this exact CI-matching dependency set still
passes 791/791 on Windows. This rules out simple dependency drift as the
cause; the failure is specific to the Linux/GitHub-Actions runner
environment itself, not reproducible locally without either a genuine
Linux environment or authenticated log access to see the real traceback.
Flagged clearly as an open item requiring operator action (grant `gh`
CLI/PAT access, or check the Actions tab directly), not guess-fixed
blind.

**Priority 5**: no hypothesis fabricated this round (H9 remains
available, explicitly not auto-started); no backtest-optimization
attempted; Legacy remains the only production engine throughout.

**Flagged for one explicit go-ahead, not acted on**: relaunching the
paper-trading process. Agent memory (not a repo file) from a prior
session documents that the process dying between sandbox restarts is
EXPECTED behavior with a standing-authorized, known-working relaunch
command -- combined with Milestone 34's Finding #1 fix now removing the
one reason relaunching would have been unsafe, this is arguably overdue.
Deliberately not done unilaterally this round: this session has
consistently required explicit sign-off for anything touching
`scripts/run_paper.py`'s running state (including a 2-line bookkeeping
fix), and starting the live paper-trading loop changes that state
materially (dormant -> actively evaluating live market data), closer to
"production behavior" than to routine read-only maintenance -- raised
for one explicit confirmation instead of assumed.

**Status**: `.github/workflows/backend-tests.yml` unmodified this round
(added Milestone 35, still failing, root cause not yet found).
`backend/app/config.py`'s Milestone-35 comment-only change is unchanged.
`CLAUDE.md` received a narrow, targeted edit (milestone count + doc
pointers + one phase-progression paragraph). No file on the live-trading
path (`RiskManager.evaluate()`/`scripts/run_paper.py`) was touched. No
orders placed; no writes to the production `backend/paper_validation.db`.
Full suite 791/791 locally (both the normal dev venv and a fresh
CI-matching venv) -- CI itself remains failing, disclosed not hidden.
Full reports, cited not duplicated: `docs/EXCHANGE_LAYER_IMPLEMENTATION_ROADMAP.md`,
...
## 75. Milestone 37: paper trader restarted and lifecycle-verified (operator-approved), a real stale-fixture bug found and fixed in the validation tooling itself, Exchange Layer Phase 0 implemented read-only-and-mocked, a genuinely missing health-check tool built, and CI's diagnosis gap closed structurally after three independent local reproductions still couldn't find the Linux-specific root cause

**Decision context**: operator approved restarting the paper-trading
service and asked for full-lifecycle verification (startup, market
data, signal generation, order execution, position management,
stop-loss, take-profit, logging, `strategy_logs`, `risk_events`,
graceful shutdown, restart recovery), then separately authorized
continuing autonomously for up to three hours against six ranked
priorities, with a hard stop for live trading, real credentials outside
what's already configured, production strategy changes, external
secrets, and destructive actions.

**Paper trader restart + lifecycle verification**. Relaunched
`scripts/run_paper.py` via the memory-verified working invocation
(`PYTHONPATH`/`DATABASE_URL` explicit, `ENABLE_SHADOW_STRATEGY_SIGNALS=true`,
`--iterations 100000 --interval-seconds 300`), first running
`scripts/migrate_paper_db.py --apply` (auto-backed-up) to bring
`paper_validation.db` to migration head. No crash at startup; iterations
ran clean with no errors across the whole session. **Signal generation /
order flow / SL-TP / concurrency guard**: re-ran the existing
`scripts/verify_signal_to_fill.py` (Milestone 33's tool, unmodified in
logic) against a fresh throwaway temp DB -- this is where a real bug
surfaced (next paragraph); after fixing it, all 21/21 checks passed,
confirming the full signal -> risk -> execute -> persist -> concurrency-
guard -> close chain still works correctly post-Milestone-34-fix, using
real current market price. **Logging**: confirmed `strategy_logs`/
`risk_events` remain genuinely dead tables (0 rows, no code writes to
them) -- this is Milestone 33's Finding #5, unchanged, not a new
regression. **Graceful shutdown / restart recovery**: confirmed by
reading `scripts/run_paper.py::main()` (not modified) -- `KeyboardInterrupt`
is caught cleanly (exit code 0, no partial-state corruption), and
`PersistentCircuitBreaker` loads its prior tripped/reason/tripped_at
state from `bot_state` at construction, so a respawned process resumes
correctly tripped rather than silently resuming trading after a crash.
**Duplicate positions**: confirmed at most one open position exists at
any time -- the concurrency guard re-queries open positions AFTER the
exit-check step (by design, per the function's own comment), so a
same-pass close-then-reopen is intentional sequential behavior, not a
concurrency violation; verified directly via `verify_signal_to_fill.py`'s
Phase 2 check once its own stale-price bug (below) was fixed.

**A real bug found in the validation tooling itself, not production
code**: `scripts/verify_signal_to_fill.py`'s hardcoded
`SYNTHETIC_TARGET = 52500.0` (chosen when real BTC price was far below
it) is now BELOW real BTC market price (~$64,716 at time of this run).
Because Phase 2's exit-check reads the real last candle close (not the
synthetic signal), the still-open synthetic position from Phase 1 was
spuriously closed via a genuine take-profit hit on real market data
before the concurrency-guard scenario the test exists to check ever
ran -- a false negative, not evidence of a duplicate-position bug in
`run_paper.py` itself (confirmed by reading the concurrency-guard code,
which behaves exactly as documented). Fixed by anchoring
`SYNTHETIC_ENTRY`/`STOP`/`TARGET` to a freshly fetched real current
price at runtime instead of a fixed historical constant -- a
maintenance fix to a diagnostic script, not a production or strategy
change; confirmed by re-running the script (21/21 checks pass).

**Priority 2 (CI diagnosis)**: ran the CI failure to ground via three
independent, genuinely distinct local reproductions this round (a fresh
venv installed purely from `requirements.txt` today, which now also
surfaced `pytest-asyncio` resolving to `0.26.0` vs. the dev venv's
`1.4.0` -- a real major-version drift, tested directly and still passed
791/791; and a genuinely fresh `git clone` of `origin/master` into a
new venv, ruling out any local-checkout contamination). All three pass
cleanly on Windows. Also checked and ruled out: a local Redis
dependency (only one stub reference exists, `routes_health.py`, no test
exercises it), real (unmocked) network calls in `backend/tests/`,
hardcoded Windows-style paths, and naive `datetime.now()` usage anywhere
in `app/`/`tests/`/`scripts/` (grep confirmed zero hits -- this
project's UTC discipline holds). The actual root cause remains
unreachable without authenticated GitHub access (`gh` CLI/PAT) to the
Linux runner's real pytest traceback -- `gh` is not installed in this
environment and the unauthenticated logs endpoint 403s even for this
public repo, confirmed directly rather than assumed. **Structural fix,
not a guess-fix**: `.github/workflows/backend-tests.yml`'s "Run test
suite" step now pipes pytest's real output through `tee` and appends
its tail to `$GITHUB_STEP_SUMMARY` (`PIPESTATUS[0]`, not `$?`, preserves
the real pass/fail exit code through the pipe) -- this makes the actual
failure text show up in the Checks UI's `output.summary` field, which
**is** readable via the public, unauthenticated check-runs API even for
a failing run. Does not fix today's failure by itself, but permanently
closes the diagnosis gap that made today's failure unreachable, for
every future CI run.

**Priority 1 (Demo Trading readiness), stopped exactly at the
credentials boundary**: implemented `OkxClient`'s Phase 0 read-only
scope from `docs/EXCHANGE_LAYER_IMPLEMENTATION_ROADMAP.md` --
`fetch_ohlcv` (delegates to the existing `CandleFetcher`, public
endpoint, no auth), `get_balance`/`get_open_positions` (real OKX v5
authenticated REST calls: `OK-ACCESS-KEY`/`OK-ACCESS-SIGN`/
`OK-ACCESS-TIMESTAMP`/`OK-ACCESS-PASSPHRASE` headers, HMAC-SHA256 of
`timestamp+method+requestPath+body`, base64-encoded, `x-simulated-trading: 1`
for demo mode) -- verified against OKX's own live v5 API docs at
implementation time via WebFetch, which also caught and corrected an
inaccuracy in the roadmap doc itself (`OK-ACCESS-KEY`, not the
previously-written `OKX-ACCESS-KEY`). `place_order`/`cancel_order`
remain `NotImplementedError` on purpose -- Phase 1 scope, its own
approval gate, not implemented here. 17 new tests
(`backend/tests/test_okx_client.py`), every one mocking `httpx.get`
directly; the signing test recomputes the expected HMAC independently
rather than re-deriving it through the same code path being tested. No
real network call has ever been made by this code or its tests. Also
built the standalone measurement harness Phase 0 calls for
(`scripts/measure_exchange_readonly_latency.py`, same pattern as
`scripts/measure_pipeline_latency.py`), which correctly refuses to run
without real `OKX_API_KEY`/`OKX_API_SECRET`/`OKX_API_PASSPHRASE`
configured (verified: it does refuse, in this environment, where none
are set) -- getting Phase 0's actual latency number is still an
explicit operator action (supply demo credentials) this milestone does
not take, consistent with the stop-condition on external secrets.
Nothing wires `OkxClient` into `scripts/run_paper.py` or any live path.

**Priority 3 (Monitoring and recovery)**: found a genuine gap --
`scripts/shadow_status.py` already answers "how close is shadow-signal
evidence accumulation to the future selector's floor," but nothing
...
throughout. Deliberately does NOT auto-restart the process -- restarting
live paper-trading state is a decision with real operational
consequences (e.g., silently resuming after a circuit-breaker trip that
tripped for a good reason), consistent with this project's own
gated-file discipline around `scripts/run_paper.py`'s running state;
this tool's job is detection, recovery stays a human-in-the-loop call.
14 new tests (`backend/tests/test_paper_trader_health_check.py`).

**Priorities 4-6**: no changes needed to `CLAUDE.md`, memory, or CI
infra beyond the structural fix above (re-checked, none of Milestone
36's touch-ups had gone stale); no fabricated hypothesis (H9 remains
available, not auto-started); Legacy remains the only production
engine throughout.

**Status**: `RiskManager.evaluate()` and `scripts/run_paper.py`
themselves untouched (the paper trader was restarted, not modified).
No real OKX credentials used anywhere (all 17 `OkxClient` tests mock
`httpx.get`); no live trading enabled; no destructive actions. Full
suite green with all new tests included (see `PROJECT_STATUS.md` item
37 for the exact count). Full reports, cited not duplicated:
`scripts/verify_signal_to_fill.py` (fixed in place, not a new doc),
