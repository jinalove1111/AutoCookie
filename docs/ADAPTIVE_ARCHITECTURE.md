...
| 40 | **The same path-argument bug generalized to write-target arguments across six more scripts; a genuine gap closed in `paper_trader_health_check.py` -- log-content scanning, not just DB state** (`scripts/_cli_path_utils.py`, `scripts/paper_trader_health_check.py`) | #78 | ✅ DONE 2026-07-20 -- operator: treat CI verification as a background task (15-min poll `Monitor` set up), continue improving research quality/monitoring/paper-trading robustness/technical debt. **Systemic fix, part 2**: follow-up grep for `Path(args.` (not just `args.db_path`) found the same Windows-vs-Linux bug on WRITE-target arguments (`--output`/`--alert-log`) in 6 more call sites (`cto_report.py`, `paper_trader_health_check.py`, `analyze_regime_performance.py`, `research_regime_delay.py`, `research_signal_selection.py`, `run_backtest.py`) -- a mis-parsed write path on real Linux would silently create a file with a literal-backslash name in the wrong location instead of erroring; never triggered in practice, fixed preventatively. Renamed `normalize_db_path_arg` -> `normalize_path_arg` before it had callers outside its own module; all 6 new call sites verified importable, not just syntax-checked. **Genuine monitoring gap closed**: health-check tool only ever checked DB state, never the trader's own stdout log CONTENT, where `run_paper.py`'s own discipline prints `ERROR:`/`WARNING:`/`ALERT:`-prefixed lines for degraded-but-not-fatal steps that never trip the circuit breaker or cause staleness. New `check_log_errors()` (errors/alerts -> UNHEALTHY, warnings alone do not) and `--log-file` flag. 11 new tests -- 2 integration tests initially had a real bug in the TEST itself (asserting on `unhealthy` without inserting a `RegimeSnapshot` row first), caught by running them, fixed by isolating each check properly. Smoke-tested against the real live trader log (CLEAN, 0 errors/warnings across 130+ iterations), then deployed -- killed two stale pre-fix watch processes, relaunched one instance with `--log-file` wired in. **Research quality**: reviewed hypothesis backlog/experiment index for staleness, both accurate, correctly left untouched; no hypothesis fabricated. `RiskManager.evaluate()`/`scripts/run_paper.py` untouched; no real credentials used or fabricated; no live trading; no destructive actions; no architecture redesign. Full suite 849/849 (838+11). See `ENGINEERING_DECISIONS.md` #78. |
| 41 | **Exchange Layer Phase 1: OKX demo order placement/cancellation implemented and live-verified end-to-end against the real OKX demo-trading API; a real order-sizing bug (OKX's undocumented minimum-notional floor) found and fixed along the way** (`scripts/verify_demo_order_lifecycle.py`, `backend/app/exchange/okx_client.py`) | #78 | ✅ DONE 2026-07-20 -- operator granted OKX Demo API Trade permission (previously Read-only). Continues Phase 0 (Milestone 37) and the already-implemented-but-previously-unverified Phase 1 code (`OkxClient.place_order`/`cancel_order`/`get_order_status`, 31 mocked unit tests, unchanged this round). **First live attempt failed**: OKX rejected with `code="1"`/`sCode="51020"` ("Your order should meet or exceed the minimum order amount") -- root cause confirmed via a live diagnostic probe: the verification script sized orders using only OKX's `minSz` (quantity-granularity only), missing OKX's separate, undocumented minimum order notional value floor. No order was created by the rejection. **Fix, scoped to the test script only** (`okx_client.py` untouched): new `compute_order_size()` helper scales size up to clear a disclosed $10 USDT floor, rounding up to the nearest `lotSz` increment; 5 new unit tests (14 -> 19). Full suite after fix: 882 (up from 877). **Second live attempt PASSED all 11 steps** against the real demo endpoint (`x-simulated-trading: 1`): order placed (ordId=`3757771015088525312`), verified live, cancelled, verified canceled, position sync `positions=[]` before/after (never filled), informational non-gating `RiskManager.evaluate()` call (approved=True). Independent post-run sweep confirmed 0 pending orders account-wide, 0 open positions, balance identical to Phase 0's original snapshot. `RiskManager.evaluate()`/`scripts/run_paper.py` untouched and not imported by any new code; no real (non-demo) endpoint reached; no real capital at risk; no architecture redesign (`BaseExchange` contract unmodified). Phase 1 explicitly does NOT include `LiveBroker` adapter implementation, SL/TP mechanism design, or any `scripts/run_paper.py` wiring (Phase 2, separately gated) -- real capital remains Phase 3. Full suite 882/882. See `ENGINEERING_DECISIONS.md` #79, full report `docs/OKX_DEMO_ORDER_LIFECYCLE_RESULTS.md`. |
